...
1. 在 disposable/local database 或已确认的 Neon branch 上，通过 migration 权限的 direct connection 按顺序执行 `db/migrations/0001_baseline.sql` 和 `db/migrations/0002_r2_image_urls.sql`。每个 migration 都是单一顶层 `DO` statement，可以把完整文件作为一次 prepared Query 发送：
   ...
   提交前也可以运行静态回归，确认 migration 与 contract 文件完整：
...
先创建唯一站长账号，再执行默认 dry-run：

```bash
rtk pnpm content:import
```

dry-run 会使用 `DATABASE_URL_UNPOOLED` 读取 owner/site/profile/文章数量并报告计划，但不会开启写事务。确认目标无文章后才运行：

```bash
rtk pnpm content:import -- --apply
```

`--apply` 会锁定文章表并再次检查空库，然后在一个事务中写入允许的默认 site/profile、categories、tags、posts、post_tags 与 audit seed。它不会创建或覆盖 owner、密码、session；目标已有文章会安全拒绝。真实 Neon 或 production 执行仍需单独批准。

## Disposable schema contract

两份 migration 在 disposable PostgreSQL 上执行后，可运行 `db/tests/schema-contract.sql` 验证单账号、状态、slug、version trigger、public view 与审计约束。该脚本用 `BEGIN`/`ROLLBACK` 包围全部 fixture；不要把它当作 migration。

## R2 图片 URL

baseline 的头像和文章封面字段默认只接受 `/images/`。使用 R2 custom domain 前，通过 direct connection 执行增量 migration；该 migration 不创建新表：
