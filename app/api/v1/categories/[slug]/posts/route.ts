...
import { listPublishedPostsByCategoryPage } from "@/lib/content/service";
...
export function GET(request: NextRequest, context: RouteContext) {
  return handleContentRead(request, async (requestId) => {
    ...
    const page = await listPublishedPostsByCategoryPage(slug, query);
    if (!page) {
      throw new ApiProblem(404, "RESOURCE_NOT_FOUND", "分类不存在");
    }
    return jsonPage(
